...
begin
  ...
  if inv.id is not null then
    insert into public.profiles (id, org_id, email, full_name, department_id, is_owner, is_purchaser, username, phone, contact_email)
    ...
    insert into public.profiles (id, org_id, email, full_name, is_owner, is_purchaser, username, phone, contact_email)
...
-- بررسی در دسترس بودن شماره/یوزرنیم قبل از ثبت‌نام
...
-- یافتن ایمیل واقعیِ حساب (auth email) از روی شماره، یوزرنیم، ایمیل تماس، یا خودِ ایمیل حساب
...
AS $$
  ...
  WHERE phone = _identifier
     OR lower(username) = lower(_identifier)
     OR lower(contact_email) = lower(_identifier)
